...
var P4WN_SQUARE_WIDTH = 50; // Ancho de los elementos del tablero
var P4WN_SQUARE_HEIGHT = 50; // Alto de los elementos del tablero
var P4WN_WRAPPER_CLASS = 'p4wn-wrapper'; // Clase para el contenedor de los componentes
var P4WN_BOARD_CLASS = 'p4wn-board'; // Clase para el contenedor del tablero
var P4WN_BOARD_CONTENT_CLASS = 'p4wn-board-content'; // Subcontenedor de board
var P4WN_MESSAGES_CLASS = 'p4wn-messages'; // Clase para los mensajes
var P4WN_LOG_CLASS = 'p4wn-log'; // Clase para mostrar los movimientos del juego
var P4WN_LOG_CONTAINER_CLASS = 'p4wn-log-container'; // Subcontenedor de log
var P4WN_CONTROLS_CLASS = 'p4wn-controls'; // Clase para los controles del juego
var P4WN_CONTROLS_CONTAINER_CLASS = 'p4wn-controls-container'; // Subcontenedor de controls
...
/****************************************************************************
* Esta función crea un elemento nuevo.
*
* - element - elemento donde se inyectara el nuevo elemento
* - childtag - elemento a crear
* - className - Clase para el nuevo elemento (opcional)
****************************************************************************/
function p4d_new_child(element, childtag, className)
{
    // Creamos el nuevo elemnto
    var child = document.createElement(childtag);
    
    // inyectamos el nuevo elemento al elemento seleccionado
    element.appendChild(child);
    
    // Verificamos si queremos asignarle una clase al nuevo elemento
    if(className !== undefined){
        // Asignamos la clase al nuevo elemento
...
/****************************************************************************
* Esta función pinta los elementos del tablero del juego
****************************************************************************/
_p4d_proto.write_board_html = function()
{
    // Obtenemos el elemento contenedor del tablero
    var div = this.elements.board;
    
    // Creamos un objeto para almacenar las piezas del tablero
    var pieces = this.elements.pieces = [];
    
    // Creamos un elemento para la tabla
    var table = p4d_new_child(div, "table");
    
    // Creamos un elemento para el tbody
    var tbody = p4d_new_child(table, "tbody");

    for (var y = 9; y > 1; y--){
        // Creamos las filas del tablero
        var tr = p4d_new_child(tbody, "tr");
        for(var x = 1;  x < 9; x++){
            // Creamos casilla del tablero
            var i = y * 10 + x;
            var td = p4d_new_child(tr, "td");
            
            // Asignamos una clase a la casilla (para el color)
            td.className = (x + y) & 1 ? P4WN_BLACK_SQUARE : P4WN_WHITE_SQUARE;
            
            // Agregamos un evento a cada casilla del tablero
            _add_event_listener(td, 'click', function(p4d, n){
                return function(e)
                {
            ...
            // Creamos un elemento imagen en cada casilla del tablero
            var img = p4d_new_child(td, "img");
            
            // Guardamos la imagen de la pieza
            pieces[i] = img;
            
            // Asignamos el path de la imagen
            img.src = P4WN_IMAGE_DIR + '/' + P4WN_IMAGE_NAMES[0];
            
            // Obtenemos el ancho del borde del tablero
            var width = document.querySelector('.p4wn-board-content').style.pixelWidth || document.querySelector('.p4wn-board-content').offsetWidth;
            
            var size = (width / 8) - 4;
            
            // Asignamos el ancho a la imagen
            img.width= size;
            
            // Asignamos el alto a la imagen
            img.height= size;
        }
    }
};
...
/****************************************************************************
* Esta función configura las opciones para pintar el tablero de juego
*
* - target - Identificador del elemento donde se pintara el tablero
****************************************************************************/
function P4wn_display(target)
{
    // Verificamos si ya instanciado el objeto
    if (! this instanceof P4wn_display){
        // Si no esta instanciado creamos una nueva instancia
        return new P4wn_display(target);
    }
    
    // Obtenemos el elemento donde se pintara el tablero y lo almacenamos en container
    var container;
    ...
    // Creamos un nuevo elemento (wrapper)
    var inner = p4d_new_child(container, 'div', P4WN_WRAPPER_CLASS);
    
    // Creamos un nuevo objeto para los elementos del tablero
    this.elements = {};
    
    // Guardamos el nuevo elemento principal (wrapper)
    this.elements.inner = inner;
    
    // Guardamos el elemento principal
    this.elements.container = container;
    
    // Creamos el elemento para el tablero
    var board = p4d_new_child(inner, "div", P4WN_BOARD_CLASS);
    this.elements.board = p4d_new_child(board, "div", P4WN_BOARD_CONTENT_CLASS);
    
    // Creamos el elemento para mostrar errores
    var log = p4d_new_child(inner, "div", P4WN_LOG_CLASS);
    this.elements.log = p4d_new_child(log, 'div', P4WN_LOG_CONTAINER_CLASS);
    
    // Creamos el elemento para los controles del tablero
    var controls = p4d_new_child(inner, "div", P4WN_CONTROLS_CLASS);
    this.elements.controls = p4d_new_child(controls, 'div', P4WN_CONTROLS_CONTAINER_CLASS);
    
    // Creamos el elemento para mostrar mensajes
    this.elements.messages = p4d_new_child(container, 'div', P4WN_MESSAGES_CLASS);
    
    this.start = 0;
    this.draw_offers = 0;
    
    // Iniciamos un juego nuevo - Funcion contenida en el motor de ajedrez
    ...
    // Asignamos el nivel por defecto del juego
...
/****************************************************************************
* Esta función asigna el alto y ancho de los elementos del tablero
*
* - square_height - alto para los elementos del tablero
* - square_width - ancho para los elementos del tablero
****************************************************************************/
_p4d_proto.render_elements = function()
{
    // Obtenemos los elementos del tablero
    var e = this.elements;
    
    // Calculamos el alto de los elementos
    // Los multiplicamos * 8 por que el tablero contien 8 casillas por lado
    var board_height = ((document.querySelector('.p4wn-board-content').style.pixelWidth || document.querySelector('.p4wn-board-content').offsetWidth) + (4 * 8)) + 'px';
    
    // Asignamos el alto al elemento principal
    e.inner.style.height = board_height;
    
    // Asignamos el alto al elemento para mostrar los movimientos
    e.log.style.height = board_height;
    
    // Asignamos el alto al elemento para los controles del juego
    e.controls.style.height = board_height;
    
    // Asignamos el alto para el borde contenedor del tablero
    e.board.style.height = board_height;
}

/****************************************************************************
* Esta función pinta el tablero de juego
*
* - id - Identificador del elemento donde se pintara el tablero
****************************************************************************/
function p4wnify(id){
    // Creamos un objeto con los elementos del juego
    var p4d = new P4wn_display(id);
    
    // Asignamos el alto y ancho de los elementos del tablero
    p4d.render_elements();
    // Creamos los elementos del tablero
...
// Prototipo para incluir metodos y propiedades al objeto
